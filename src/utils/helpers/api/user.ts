...
export const getUser = async ({ uid, type }: { [key: string]: string }) => {
  const { data }: { data: Patient | Doctors } = await axios(
    `https://med-app-89f4.onrender.com/api/user?uid=${uid}&type=${type}`
  );
  return data;
};

export const addPatient = async (userDataBody: Patient) => {
  const result = await axios.post(`https://med-app-89f4.onrender.com/api/user?type=patient`, {
    ...userDataBody
  });
  return result;
};
...
export const signUpDoctor = ({ data, setStatus }: SignUpDoctorParams) => {
  ...
  createUserWithEmailAndPassword(auth, email, password)
    .then(async ({ user: { uid } }) => {
      ...
      await axios.post(`https://med-app-89f4.onrender.com/api/user?type=doctor`, finalData);
      setStatus('success');
    })
    .catch((error) => {
      setStatus(error);
