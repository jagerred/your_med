...
export const getAccessStatus = async ({ patientId, uid }: GetAccessStatusParams) => {
  const { data } = await axios(
    `https://med-app-89f4.onrender.com/api/access?patientId=${patientId}&&doctorId=${uid}`
  );
...
export const giveAccess = async ({ patientId, doctorId, durationDays }: GiveAccessParams) => {
  ...
  const result = await axios.post('https://med-app-89f4.onrender.com/api/access', accessData);
  return result;
};
