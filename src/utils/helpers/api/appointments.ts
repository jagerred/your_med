...
export const getPatientAppointments = async ({ uid, page }: GetPatientAppointmentsParams) => {
  const appointments = await axios(
    `https://med-app-89f4.onrender.com/api/appointments?type=${'patient'}&uid=${uid}&page=${page}`
  );
  return appointments.data;
};

export const getAdminAppointment = async (): Promise<DoctorAppointmentsForAdmin> => {
  const appointments = await axios('https://med-app-89f4.onrender.com/api/adminAppointments');
  return appointments.data;
};

export const getDoctorAppointments = async ({ uid, date }: GetDoctorAppointmentsProps) => {
  const appointments = await axios(
    `https://med-app-89f4.onrender.com/api/doctorAppointments?uid=${uid}&date=${date}`
  );
  return appointments.data;
};
...
export const changeAppointmentStatus = async (
  ...
  try {
    await axios.put(`https://med-app-89f4.onrender.com/api/appointments?id=${id}`, {
      status
    });
...
export const makeAnAppointment = async (
  ...
  try {
    await axios.post('https://med-app-89f4.onrender.com/api/appointments', preparedAppointment);
